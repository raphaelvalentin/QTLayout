...
#        self.g = -J_T.dot(self.residual)
        self.A = self.Jacobian
        self.g = -self.residual
        #print 'A = ', self.A
        #print 'b = ', self.g
        m = len(self.A[0])
        self.I =array([[0. for i in xrange(m)] for i in xrange(m) ])
        for i in xrange(m):
            self.I[i][i] = 1.
...
#        self.dX = self.solve(self.A + self.tau*self.A.diag(),self.g)
        ...
        self.dX = self.solve(r.T.dot(r)+self.tau*np.diag(r.T.dot(r)), q.transpose().dot(self.g))
        self.dX = array([[float(self.dX[i][0])] for i in xrange(len(self.dX))])
...
#        self.X += self.dX
        for i in xrange(len(self.X)):
            self.X[i][0] = self.X[i][0] + float(self.dX[i][0] )
        raw_input()
...
#        self.dL = (0.5*(self.dX.transpose().dot(self.tau*self.A.diag().dot(self.dX) + self.g)))[0][0]
        J_T = self.Jacobian.transpose()
        self.B = array([[float(r[i][j]) for j in xrange(len(r[0]))] for i in xrange(len(r))])
        self.h = self.g
        self.dL = (0.5*(self.dX.transpose().dot(self.tau*self.B.diag().dot(self.dX) + self.h)))[0][0]
...
class LMA(levmar):
    def __init__(self, X, residual, Jacobian):
        ...
        self.config(eps_gradient=1e-8, eps_x_step=1e-8)
        self.config(damping=(0.5, 2.0))
...
def echo(obj, step):
    ...
    if step==3:
        ...
        for v in obj.bstep.X.transpose()[0].tolist():
            print '%11.7e'% v, 
        print '%11.7e'%obj.bstep.norm_residual,
        print
